---
title: "NKLegacy Architecture: Boot to Shell"
...
<Note>
  The kernel binary is always placed at **physical address 0x100000 (1 MB)**. NKBootman Stage 2 loads the flat binary from disk sector 17 into a temporary buffer at 0x10000, then uses `rep movsd` to relocate it to 0x100000 before jumping there. When GRUB loads the ELF instead, it reads the `PT_LOAD` segment whose load address is 0x100000 (as declared in `linker.ld`) and maps it directly — no relocation step is needed.
</Note>
